    ...
    <div className="animate-in">
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{realTasks.length} tâches au total</div>
        </div>
      </div>
      ...
      {/* KANBAN — tâches uniquement */}
      {tab === 'kanban' && (
        <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '16px' }}>
          {STATUSES.map(status => {
            const colTasks = realTasks.filter(t => t.status === status.value)
            return (
              <div key={status.value} className="kanban-col">
                <div className="kanban-col-header">
                  ...
                  <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', background: status.bg, color: status.color, padding: '2px 7px', borderRadius: '100px', border: `1px solid ${status.border}` }}>{colTasks.length}</span>
                </div>
      ...
      {/* MEMBERS — tâches uniquement, directement assignées */}
      {tab === 'members' && (
        <div>
          ...
          {(memberFilter === 'all' ? users : users.filter(u => u.id === memberFilter)).map(u => {
            const myTasks = realTasks.filter(t => (t.assigned_users || []).includes(u.id))
              ...
              <div key={u.id} style={{ marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px', paddingBottom: '10px', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{active.length} active{active.length > 1 ? 's' : ''} · {done.length} terminée{done.length > 1 ? 's' : ''}</div>
                  </div>
                </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--border)' }}>
                          {['Tâche', 'Projet › Module', 'Statut', 'Urgence'].map(h => (
                            <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: '11px', fontWeight: '600', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', background: 'var(--surface-2)' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
